...
@section('content')
    ...
    <div class="card mb-3">
        <div class="card-body">
            <div class="row flex-between-center">
                <div class="col-md">
                    <h5 class="mb-2 mb-md-0">Create New Page</h5>
                </div>
                <div class="col-auto">
    ...
    @include('admin.cms-builder.partial.form', [
        'action' => route('admin.cms-builder.pages.store'),
        'method' => 'POST',
    ...
    <script>
        $(document).ready(function() {
            ...
            window.addFieldToSection = function(sectionId, fieldType) {
                            ...
                            <div class="d-flex align-items-center gap-3">
                                <div class="form-check form-switch d-flex align-items-center gap-1 m-0">
                                    <input class="form-check-input"
                                        type="checkbox"
                                        id="${hiddenId}"
                                        name="section[${sectionId}][fields][${index}][is_hidden]"
                                        value="1">
                                    <label class="form-check-label mb-0" for="${hiddenId}">
                                        Hidden
                                    </label>
                                </div>
                                <!-- Required -->
                                ...
                                ${fieldType === 'input' ? `
                                                        <div class="form-check form-switch d-flex align-items-center gap-1 m-0">
                                                            <input class="form-check-input is-related-toggle" 
                                                                type="checkbox"
                                                                id="${relatedId}"
                                                                name="section[${sectionId}][fields][${index}][is_related]"
                                                                value="1">
                                                            <label class="form-check-label mb-0" for="${relatedId}">
                                                                Is Related
                                                            </label>
                                                        </div>` : ''}
                ...
                if (fieldType === 'input') {
                    fieldHtml += `<input class="form-control form-control-sm mt-2"
                        ...
                        placeholder="Text" type="text" disabled>
                        <div class="related-dropdowns mt-2 d-none align-items-center gap-3" id="dropdown-container-${sectionId}-${index}">
                            ...
                            <div class="d-flex gap-2 flex-grow-1 mt-2">
                ...
                <div class="accordion-item mb-2" id="${newSectionId}">
                    ...
                    <div class="accordion-collapse collapse" id="${newSectionId}-collapse" aria-labelledby="heading-${newSectionId}"
                        ...
                        <div class="accordion-body">
                            ...
                            <!-- IMPORTANT: Add fields-drop-zone and drag-fields-zone to duplicated section -->
                            <div class="fields-drop-zone" data-section-id="${newSectionId}">
                                <hr class="text-200">
                                <div class="row gx-2" id="fields-container-${newSectionId}">
                                    ${fieldsHTML}
                                </div>
                                <div class="drag-fields-zone mt-3" data-section-id="${newSectionId}">
                                    <i class="fas fa-plus-circle me-1"></i>
                                    Drag & Drop fields here
                                </div>
                            </div>
            ...
            $(document).on('change', '.cms-page-select', function() {
                ...
                if (!pageId) {
                    sectionSelect.html('<option value="">-- Select Section --</option>').prop('disabled',
                        true);
                    return;
                }
            ...
            $(document).on('change', '.cms-section-select', function() {
                ...
                const $fieldSelect = $(
                    `.cms-field-select[data-section-id="${parentSectionId}"][data-field-index="${fieldIndex}"]`
                    );

                if (!sectionId) {
                    $fieldSelect.empty().append('<option value="">-- Field --</option>').prop('disabled',
                        true);
                    return;
                }

                $.ajax({
                    url: `/admin/cms-builder/cms-sections/${sectionId}/fields`,
                    type: 'GET',
                    success: function(data) {
                        ...
                        const selectedField = $fieldSelect.data('selected-field');
                        if (selectedField) {
                            $fieldSelect.val(selectedField);
                        }
                    },
                    error: function() {
                        console.error('Failed to fetch fields for section ' + sectionId);
                    }
                });
            });
        });
